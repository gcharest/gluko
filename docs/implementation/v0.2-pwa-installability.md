...
### 3.1 Database Lifecycle Handlers (NEW for v0.2)

Add to `useIndexedDB.ts` composable:

```typescript
function setupDatabaseLifecycleHandlers(db: IDBDatabase) {
  // Handle version changes (multi-tab scenario)
  db.onversionchange = (event: IDBVersionChangeEvent) => {
    db.close()
    // Notify UI: "New version available. Please refresh."
    showRefreshPrompt()
  }
  ...
  db.onclose = (event: Event) => {
    ...
    // Attempt reconnection on next user action
  }
}
...
openReq.onblocked = (event: IDBVersionChangeEvent) => {
  console.warn('Database upgrade blocked - close other tabs')
  showAlert('Close all other tabs with this site open')
}
...
- [ ] Multi-tab test: Open app in tab A → open in tab B with updated code
  - Verify tab A receives `onversionchange` event
  - Verify tab A shows refresh prompt
  - Verify tab B successfully upgrades schema
- [ ] Verify `onblocked` shows if old tab keeps locks
- [ ] Simulate browser shutdown and verify graceful reconnection
...
1. **nutrientsFile.spec.ts**:
   ...
   - **NEW**: Data validation on deserialization (invalid structures rejected)
...
- **NEW**: Database lifecycle handlers (`onversionchange`, `onblocked`, `onclose`)
- **NEW**: Data validation on deserialization (invalid structures gracefully rejected)
- **NEW**: Transaction atomicity for multi-operation scenarios
- **NEW**: QuotaExceededError recovery options
