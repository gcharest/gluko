...
- **Database lifecycle**: Must implement onversionchange, onblocked, onclose handlers (multi-tab support per MDN 2025 best practices)
...
## Database Lifecycle & Best Practices (2025)

### Multi-Tab Support: `onversionchange` Handler

When user opens app in new tab with updated code (new DB version), old tabs must receive notification and close their connections to allow schema upgrade:

```typescript
// In useIndexedDB composable
db.onversionchange = (event: IDBVersionChangeEvent) => {
  db.close()
  // Notify user: "New version available. Please refresh."
  // Prevents stale tabs from holding old schema locks
}
```

### Blocked Upgrades: `onblocked` Handler

If user has old tab open with read locks, new version's upgrade blocks until those locks release:

```typescript
const openReq = indexedDB.open(DB_NAME, newVersion)

openReq.onblocked = (event) => {
  console.log("Database upgrade blocked - close other tabs")
  // Show UI: "Close all other tabs with this site open"
}
```

### Unexpected Closure: `onclose` Handler

Handle unexpected database closure (browser shutdown, permission revoked, etc.):
...
- [ ] Multi-tab test: Verify `onversionchange` fires when new version opened
- [ ] Verify user sees refresh prompt on old tab
- [ ] Verify new tab can upgrade schema after old tab closes

### Data Validation on Deserialization

Always validate data structure when reading from IndexedDB to catch schema mismatches:

```typescript
function validateMealRecord(data: unknown): MealRecord | null {
  ...
  if (
    ...
    console.warn('Invalid meal record:', record)
    return null  // Or skip if in list iteration
  }

  return record as MealRecord
}
```

**Impact**: Prevents silent data corruption if schema evolves or data becomes corrupted.
...
1. Test `useIndexedDB` composable:
   ...
   - Test index queries
   - **NEW**: Test data validation on deserialization (invalid structures rejected)
   - **NEW**: Test transaction atomicity for multi-operation scenarios
...
3. Integration tests:
   ...
   - **NEW**: Test multi-tab version change scenario (onversionchange handler)
   - **NEW**: Test QuotaExceededError handling with graceful recovery
